...
namespace Floozy
{
    public class AssertContext<TPageModel> where TPageModel : class, new()
    {
        ...
        public PageContext<TPageModel> TitleIs(string text)
        {
            if(_pageContext.WebDriver.Title != text)
                throw new InvalidOperationException($"Assertion failed - title was '{_pageContext.WebDriver.Title}', not '{text}'");
            return _pageContext;
        }
    }
}